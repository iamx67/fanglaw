...
import { getPreyProfileById, isRunnerPreyProfile } from "../config/preyProfiles.js";
import { AuthStoreError, accountsService, charactersService, } from "../services/index.js";
...
export class CatRoom extends Room {
    constructor() {
        ...
        this.nextMoveAllowedAt = new Map();
        this.lastSprintAt = new Map();
        this.lastStaminaTickAt = new Map();
        this.sprintRequiresRepress = new Map();
        this.playerZoneToPreyId = new Map();
        this.playerZoneCooldownUntil = new Map();
    ...
    onCreate() {
        ...
        this.onMessage("sprint-release", (client, _message) => {
            const playerId = this.sessionToPlayerId.get(client.sessionId);
            if (!playerId) {
                return;
            }
    ...
    onJoin(client, _options, auth) {
        const authData = requireGameAuthContext((client.auth ?? auth));
    ...
    trySearchPrey(client, playerId, player, message) {
    ...
    updateRunnerPrey(preyId, prey, preyProfile, spawnMeta, now) {
    ...
    updateBirdPrey(prey, preyProfile, spawnMeta, now) {
        if (preyProfile.behaviorType !== "bird") {
            return;
        }
    ...
    findNearestPlayerFromList(players, x, y) {
        let nearestPlayer = null;
        let nearestDistance = Number.POSITIVE_INFINITY;
    ...
    findConnectedPlayersWithinRadius(x, y, radius) {
        const players = [];
        for (const player of this.state.players.values()) {
            if (!player.connected) {
                continue;
    ...
    findConnectedPlayerById(playerId) {
        if (!playerId) {
            return null;
        }
        const player = this.state.players.get(playerId);
    ...
    getBirdRelevantPlayers(prey, preyProfile, spawnMeta, triggeringPlayer = null) {
    ...
    getBirdTargetPlayer(prey, spawnMeta, relevanceRadius, nearbyPlayers, triggeringPlayer) {
    ...
    findBestPreyFleeStep(preyId, prey, _preyProfile, spawnMeta, playerX, playerY) {
    ...
    getSearchZoneUnavailableUntil(playerId, searchZoneId, now) {
        const playerZoneKey = buildPlayerSearchZoneKey(playerId, searchZoneId);
    ...
    setSearchZoneCooldown(playerId, searchZoneId, durationMs, now) {
    ...
    resolveBirdDetectionForPlayerMovement(player, now) {
        for (const [preyId, prey] of this.state.prey.entries()) {
    ...
    teleportBirdPrey(preyId, prey, preyProfile, spawnMeta, targetPlayer, connectedPlayers, now) {
        if (preyProfile.behaviorType !== "bird") {
            return;
        }
        ...
        const nextPosition = findBirdPerchPosition(zone, preyProfile, this.state.prey, preyId, targetPlayer, connectedPlayers);
        if (nextPosition) {
    ...
    resolvePreyInteractionAt(playerId, player) {
        for (const [preyId, prey] of this.state.prey.entries()) {
            ...
            if (prey.state === "alive") {
                ...
                const preyProfile = getPreyProfileById(spawnMeta?.preyProfileId || prey.kind);
                const updatedCharacter = charactersService.addSkillXp(playerId, player.name, HUNTING_SKILL_ID, Math.max(0, preyProfile.xpReward));
...
function getSearchSuccessChance(zone, preyProfile) {
    void zone;
    void preyProfile;
    return 1;
}
function getPreyProfileForSearchZone(zone) {
    return getPreyProfileById(zone.preyProfileId || zone.preyKind);
}
function getPreyBehaviorIntervalMs(preyProfile) {
    if (isRunnerPreyProfile(preyProfile)) {
...
function getPreyPerchTimeoutMs(preyProfile) {
    if (preyProfile.behaviorType === "bird") {
        return preyProfile.perchTimeoutMs;
    }
    return 0;
}
function findPreySpawnPosition(player, zone, preyProfile, preyMap, connectedPlayers) {
    if (preyProfile.behaviorType === "bird") {
...
function findBirdSpawnPosition(player, zone, preyProfile, preyMap, connectedPlayers) {
    const size = worldConfig.gridCellSize;
...
function findBirdPerchPosition(zone, preyProfile, preyMap, ignoredPreyId, targetPlayer, connectedPlayers) {
    const size = worldConfig.gridCellSize;
    const leashRadius = Math.max(1, preyProfile.teleportRadiusCells) * size;
    const expandedBounds = getExpandedPreySearchZoneBounds(zone, leashRadius);
    const minGridX = Math.ceil(expandedBounds.left / size);
...
function compareBirdPerchCandidates(left, right) {
...
function buildPlayerSearchZoneKey(playerId, searchZoneId) {
    return `${playerId.trim()}::${searchZoneId.trim()}`;
}
function getExpandedPreySearchZoneBounds(zone, margin) {
    return {
...
function isWorldPositionInsideExpandedPreySearchZone(zone, x, y, margin) {
...
function toCharacterWorldSnapshot(player) {
    return {
...
function requireGameAuthContext(auth) {
