...
func _create_match_make_request(
	...
	if response.get('code') != null:
		...
		return
	var room_data = _extract_room_data(response)
	if room_data == null:
		promise.reject(str("Invalid matchmaking response: ", response))
		return
...
func _extract_room_data(response: Dictionary):
	if response.has("room") and response["room"] is Dictionary:
		return response["room"]

	if response.has("name") and response.has("roomId") and response.has("processId"):
		return {
			"name": response["name"],
